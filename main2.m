function [F, T, obj]=main2(X,numClusters,r1Temp,r2Temp)
...
lambda = sqrt(ones(numViews,1)/(numViews));
...
F=Uf*Vf';
% F = zeros(numSamples,numClusters);
...
K=cell(numViews,1);
gamma = sqrt(ones(numViews,1)/(numViews));
for v=1:numViews
...
%        obj1(iter) = trace(r1Temp*F'*T);
%        obj2(iter)=trace(r2Temp*F'*HR_sum);
